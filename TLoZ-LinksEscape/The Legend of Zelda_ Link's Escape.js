...
async function playMusic() {
  playback = playTune(mPart1);
  playback = playTune(bPart1);
  setTimeout(() => {
    playback = playTune(mPart2);
    playback = playTune(bPart2);
...
async function musicLoop() {
  ...
  playback = playTune(mPart3);
  playback = playTune(bPart3); }
  setTimeout(async () => {
    if (isEndGame) {  }
    else {
    playback = playTune(mPart4);
    playback = playTune(bPart4); }
    setTimeout(async () => {
      if (isEndGame) {  }
      else {
      playback = playTune(mPart5);
      playback = playTune(bPart5); }
      setTimeout(async () => {
        if (isEndGame) {  }
        else {
        playback = playTune(mPart6);
        playback = playTune(bPart6); }
        setTimeout(musicLoop, 4000);
      }, 4000);
    }, 4000);
  }, 4000);
...
async function playEndGame() {
  if (!isEndGame) { return; }
  playback = playTune(ePart1);
  playback = playTune(eBPart1);
  setTimeout(async () => {
    if (!isEndGame) { return; }
    playback = playTune(ePart2);
    playback = playTune(eBPart2);
    setTimeout(async () => {
      if (!isEndGame) { return; }
      playback = playTune(ePart3);
      playback = playTune(eBPart3);
      setTimeout(async () => {
        if (!isEndGame) { return; }
        playback = playTune(ePart4);
        playback = playTune(eBPart4);
        setTimeout(playEndGame, 3000);
      }, 5333);
    }, 5333);
  }, 5333);
}
